"""Cookie-backed account sessions and request identity for QueryLens using Supabase PostgreSQL."""
...
def _issue_session(user_id: str):
    ...
    with get_supabase_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("DELETE FROM public.auth_sessions WHERE expires_at <= %s", (now,))
            cur.execute(
                "INSERT INTO public.auth_sessions(token_hash, user_id, expires_at, created_at) VALUES (%s, %s, %s, %s)",
...
def _user_for_token(token: str | None):
    ...
    with get_supabase_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                """
                SELECT u.id, u.email, u.display_name FROM public.auth_sessions s
        ...
        with get_supabase_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    ...
                    VALUES (%s, %s, %s, %s, %s)
                    """,
                    (user_id, email, display_name, _hash_password(payload.password), now)
                )
                cur.execute("SELECT id, email, display_name FROM public.auth_users WHERE id = %s", (user_id,))
                row = cur.fetchone()
...
async def signin(payload: LoginCredentials, request: Request, response: Response):
    ...
    with get_supabase_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT * FROM public.auth_users WHERE LOWER(email) = LOWER(%s)", (email,))
            row = cur.fetchone()
...
async def signout(request: Request, response: Response):
    ...
    if token:
        ...
        with get_supabase_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM public.auth_sessions WHERE token_hash = %s", (digest,))
